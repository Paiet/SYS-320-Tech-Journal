...
# Description: script for local secuirty checks
...
# Checking password max days policy
...
# Checking the passowrd min days between changes
...
# Checking the password warning age

pwarn=$(egrep -i '^PASS_WARN_AGE' /etc/login.defs | awk '{ print $2 }')

echo ""

checks "Password change warning" "7" "${pwarn}"

# Checking the SSH UserPam configuration

chkSSHPAM=$(egrep -i "^UusePAM" /etc/ssh/sshd_config | awk ' { print $2 } ' )

echo ""

checks "SSH UsePAM" "yes" "${chkSSHPAM}"

# Checking perms on user home dirs
